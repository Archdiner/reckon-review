...
| `guard.ts` | `study/src/guard.ts` | The containment assertion. Written after the study's own overlap check was found undetectable and retracted, and it handles two cases three separate reimplementations here did not: it strips `diffDigest`'s synthetic markers, and it scopes shingles per line because the digest drops context lines so distant lines become adjacent. Reinventing it produced a guard that could not fire, then one that fired on legitimate shared prose, then one that fired on the digester's own header. |
