...
export function formatThreeArmReport(rows: ThreeArmRow[]): string {
  ...
  L.push('');
  L.push('## Methods result: the rubric is insensitive to surface form');
  L.push('');
  L.push('This is a validation of the instrument, and it stands independently of any finding.');
  L.push('');
  L.push('The paraphrase arm is the real record carrying the model\'s headings, bullets, code');
  L.push('formatting and vocabulary, with no information added. It scores the same as the raw');
  L.push('record. A scorer that rewarded polish, fluency or familiar structure would have moved;');
  L.push('this one did not.');
  L.push('');
  L.push('That closes the most common objection to an LLM-judged study — "your grader is just');
  L.push('rewarding text that looks like model output" — with a measurement rather than an');
  L.push('assurance. The grader is responding to content, because holding content fixed and');
  L.push('changing everything else produced no change in score.');
  L.push('');
